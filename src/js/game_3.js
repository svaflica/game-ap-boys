...
localStorage.setItem('PlayerTwo', 'CD');
canvas.width = 1024
canvas.height = 567
...
if (window.localStorage.PlayerOne != undefined) {
...
function animate() {
    ...
    if (a) {
        status_game('Третье испытание: CD<br>Нажмите Enter, чтобы продолжить...')
    }
    if (keys.enter.pressed && a) {
        a = null
        keys.enter.pressed = false;
        document.querySelector('.c-fufel').remove()
        blockControlE = false;
        blockControlP = false;
        decreaseTimer();
    }
    ...
    if (player.health <= 0 || enemy.health <= 0) {
        if (player.health <= 0 && enemy.health > 0) {
            ...
            winner_text = 'DevOps победил, но не сломил...'
        }

        if (enemy.health <= 0 && player.health > 0) {
            enemy.switchSprite('death');
            winner_text = "Теперь ты senior docker container"
        }
...
setInterval(function(){
    if (!blockControlE && !enemy.dead && !player.dead) {
        delta_1 = enemy.position.x - player.position.x

        if (delta_1 > 0 && Math.abs(delta_1) < 200) {
            enemy.attacks();
        }
